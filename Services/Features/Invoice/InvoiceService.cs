...
public class InvoiceService(IServiceProvider services,IAuth auth, IDbContextFactory<ApplicationDbContext> dbContextFactory) : DbServiceBase<AppDbContext>(services), IInvoiceService 
{
    #region Queries
    ...
    public async virtual Task Create(CreateInvoiceCommand command, CancellationToken cancellationToken = default)
    {
        ...
        var merchant = dbContext.Merchants.First(x=>x.Id == command.InvoiceRequest.MerchantId); 
