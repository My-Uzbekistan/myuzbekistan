...
public class FavoriteApiViewConverter : JsonConverter<FavoriteApiView>
{
    public override FavoriteApiView? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        // Используем стандартную десериализацию
        return JsonSerializer.Deserialize<FavoriteApiView>(ref reader, options);
    }

    public override void Write(Utf8JsonWriter writer, FavoriteApiView value, JsonSerializerOptions options)
    {
...
public class ContentApiViewConverter : JsonConverter<ContentApiView>
{
    public override ContentApiView? Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        // Используем стандартную десериализацию
        return JsonSerializer.Deserialize<ContentApiView>(ref reader, options);
    }

    public override void Write(Utf8JsonWriter writer, ContentApiView value, JsonSerializerOptions options)
    {
        writer.WriteStartObject();
