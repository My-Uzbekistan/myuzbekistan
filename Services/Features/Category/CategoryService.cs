...
public class CategoryService(IServiceProvider services) : DbServiceBase<AppDbContext>(services), ICategoryService 
{
    ...
    [ComputeMethod]
    public async virtual Task<CategoryView> Get(long Id, CancellationToken cancellationToken = default)
    {
        await Invalidate();
        await using var dbContext = await DbHub.CreateDbContext(cancellationToken);
        ...
        return category == null ? throw new ValidationException("CategoryEntity Not Found") : category.MapToView();
    }
