...
[Route("api/v2/contents")]
[ApiController]
public class V2ContentController(ICategoryService categoryService, IContentService contentService, ISessionResolver sessionResolver, ICommander commander) : ControllerBase
{
    ...
    public async Task<IActionResult> GetContent(long contentId, CancellationToken cancellationToken)
    {
        ...
        {
            var userId = User.Id();
            contentDto = await _contentService.GetContent(contentId, userId, cancellationToken,true);
