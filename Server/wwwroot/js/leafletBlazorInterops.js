...
window.leafletBlazor = {
    ...
    getAdress: function (Latlng) {
        let resolve, reject;
        let promise = new Promise((res, rej) => {
            resolve = res;
            reject = rej;
        });
        fetch('https://nominatim.uztelecom.uz/reverse?format=json&lon=' + Latlng.lng + '&lat=' + Latlng.lat).then(function (response) {
            return response.json();
        }).then(function (json) {
            
            let house = json.address.house_number == undefined ? '' : json.address.house_number;
            let road = json.address.road == undefined ? '' : ((house == '' ? '' : ',') + json.address.road);
            let hamlet = json.address.hamlet == undefined ? '' : ((house == '' && road == '' ? '' : ',') + json.address.hamlet);
            let suburb = json.address.suburb == undefined ? '' : ((house == '' && road == '' && hamlet == '' ? '' : ',') + json.address.suburb);
            let county = json.address.county == undefined ? '' : ((house == '' && road == '' && hamlet == '' && suburb == '' ? '' : ',') + json.address.county);
            let city = json.address.city == undefined ? '' : ((house == '' && road == '' && hamlet == '' && suburb == '' && county == '' ? '' : ',') + json.address.city);
            resolve(house + road + hamlet + suburb + county + city);
        })
        return promise;
    },
    // Function to zoom to a specific location
    zoomToLocation: function (mapId, lat, lng, zoomLevel, text, iscar) {
        maps[mapId].setView([lat, lng], zoomLevel);
        if (text != "") {
            L.popup({
                offset: [0, -33]
            }).setLatLng([lat, lng])
    ...
    addRoute: function (mapId, routes, gpsroutes) {
        var LatLngs = [];
        var url = 'https://osrm.uztelecom.uz/route/v1';
        routes.forEach(pts =>
            LatLngs.push(pts));
        var plan = L.Routing.plan(LatLngs, {
            createMarker: function (i, wp) {
                var myIcon;
                if (i === 0) {
                    myIcon = L.icon({
                        iconUrl: '/Images/pin_a.png',
                        iconAnchor: [15.5, 40],
                        popupAnchor: [0, -40],
                        iconSize: [30, 41],
                        draggable: false
                ...
                return L.marker(wp.latLng, { icon: myIcon });
            }
        });
        L.Routing.control({
        ...
        if (gpsroutes != null) {
            var GpsLatLng = gpsroutes.map(pts => {
                return {
                    latLng: L.latLng(pts.lat, pts.lng), show: false
                }
            })
            let result = [];
            GpsLatLng.forEach(arr => {
                if (result != null && !result.some(x => x.latLng.lat === arr.latLng.lat && x.latLng.lng === arr.latLng.lng)) {
                    result.push(arr);
                }
            });
...
function createIcon(icon) {
    return L.icon({
        ...
        iconSize: icon.size?.width ? L.point(icon.size.width, icon.size.height) : null,
        iconAnchor: icon.anchor?.x ? L.point(icon.anchor.x, icon.anchor.y) : null,
...
function addRoute(mapId, routes) {
    var LatLngs = [];
    routes.forEach(pts =>
        LatLngs.push(pts));
    debugger
